...
- **Two kinds of reader, and the rule is stated once, on `DatedOnboardingFields`.** "Has this talent ever signed, and when" reads the flat columns as they stand: broadcast filters, image rights, early-bird (which pairs them with the year stamp), the fiche talent's dossier rail. "Is year Y's dossier done" goes through `onboardingFieldsForYear`, or through the dossier rows when the question is about a year that is not the current one. **Anything that names one document reads the dossier row**, never the projection: the PDF worker, `/settings/documents`, the staff archive export, onboarding velocity.
...
**Its rendered PDF belongs to the dossier, not to the talent** (`Onboarding_Record.rulesFilePath`, key `documents/{talentId}/rules-{schoolYear}.pdf`). Version-pinning protects the *wording* of a signed document; this protects the *document*. One key per talent meant the second year's render overwrote a PDF a legal guardian had already co-signed for a minor, with no way to rebuild it, and it silently dropped the previous year out of `/settings/documents` and the staff archive. Two consequences to keep: an `OnboardingPdfJob` carries the `schoolYear` it renders, because a job can be queued or retried long after the signature that asked for it and "the current dossier" will have moved; and both reset paths (`resetTalentToImport`, `anonymizeTalent`) collect the keys from **every** dossier row, since deleting the rows is what drops the references. A document kind declares `scope: 'account' | 'dossier'` in `ONBOARDING_DOCUMENTS` rather than naming a `Talent` column, which is what let the règlement keep pointing at a per-talent artifact after it became annual.
...
- **`infra/documentRenderer.ts`** - the one browser-render path: PDFs for what gets printed, PNGs for what gets looked at, both over the same page setup so a preview cannot disagree with the document it previews. Owns the page lifecycle and **blocks the network**, so no caller can render a stored design with that switched off by forgetting to switch it on. Fonts therefore carry their own bytes (`templates/fonts.ts`, `@font-face` built from the `@fontsource` packages with `?inline`)
