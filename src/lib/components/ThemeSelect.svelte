...
<div class="grid gap-2">
	<Popover.Root bind:open>
			...
			{#snippet child({ props })}
				...
				>
					<div class="flex items-center gap-2 truncate">
						...
						{:else}
							<span class="text-muted-foreground">Sélectionner ou créer un thème...</span>
						{/if}
					</div>
		...
		<Popover.Content class="w-[--bits-popover-anchor-width] p-0" align="start">
			<Command.Root>
				<Command.Input placeholder="Rechercher un thème..." bind:value={searchValue} />
				<Command.List class="max-height-[300px] overflow-y-auto">
					<Command.Empty>
						...
						{:else}
							<p class="py-6 text-center text-sm text-muted-foreground">Aucun thème trouvé.</p>
						{/if}
					</Command.Empty>
