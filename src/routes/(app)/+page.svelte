				...
				{#each data.sessions as session}
					<TableRow>
						...
						<TableCell>
							{#if session.activity}
								<div class="flex items-center">
									<Activity class="mr-2 h-4 w-4 text-muted-foreground" />
									{session.activity.nom}
									{#if session.activity.difficulte}
										<span
											class="ml-2 inline-flex h-6 w-16 items-center justify-center rounded-full bg-muted px-2 py-1 text-xs font-medium"
										>
							...
							{:else}
								<span class="text-sm text-muted-foreground">Non définie</span>
							{/if}
						</TableCell>
						<TableCell>
