...
| 9 | Pas d'URL de production ou staging dans les fichiers de test (Supabase, Epitech, etc.) | Erreur | §3.2 — Règles |
...
- **Commentaires inline** — `isComment()` ne gère pas les commentaires en fin de ligne (`const x = 1 // prod.supabase.co`). La règle 9 pourrait donc signaler une URL dans un commentaire inline — en pratique ce n'est pas un problème.
