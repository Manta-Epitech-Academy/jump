...
async function main() {
  ...
  try {
    ...
    const answer = await rl.question(
      'Have you manually created the "student_migrated" relation field in the participations collection? (y/n): ',
    );

    if (answer.toLowerCase() !== 'y') {
      console.log(
        'Aborting. Please create the field first and then run this script again.',
      );
      process.exit(0);
    }
    ...
    if (records.length === 0) {
      ...
      for (let i = 0; i < records.length; i += batchSize) {
        ...
        console.log(
          `   Processing batch ${i + 1}-${Math.min(i + batchSize, records.length)}...`,
        );
    ...
    console.log(
      "4. (Optional) Once verified, you can delete the 'old_students' field.",
    );
    console.log('='.repeat(50) + '\n');
  } catch (err: any) {
