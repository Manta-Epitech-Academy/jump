...
function normalizeHeader(h: string): string {
  ...
  if (clean.includes('etudes') || clean === 'niveau' || clean === 'level')
    return 'niveau';
...
export function parseEventImportCsv(
  csvString: string,
): Promise<CsvEventImport> {
  return new Promise((resolve, reject) => {
    Papa.parse(csvString, {
      ...
      complete: (results) => {
        ...
        const campaignKey =
          Object.keys(firstRow).find((k) => k.includes('campagne')) ||
          'nom de la campagne';
