<script lang="ts">
  ...
  // surfaces keep the same labels and default and can't drift apart.
  //
  // The choice persists per browser: a week layout is a display preference, and
  // its only sensible default is "what you last picked". localStorage (not a DB
  // column) is the right scope for a per-device, client-only cosmetic toggle.
  // It's read after mount (localStorage is client-only, so SSR renders the
  // default first) and written on every change; the `loaded` gate keeps the
  // first write from clobbering the stored value before it's read back.
