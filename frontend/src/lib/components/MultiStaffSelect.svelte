		...
		<Popover.Content class="w-[--bits-popover-anchor-width] p-0" align="start">
			<Command.Root>
				...
				<Command.List class="max-h-75 overflow-y-auto">
					<Command.Empty>
						<p class="py-6 text-center text-sm text-muted-foreground">Aucun staff trouvé.</p>
					</Command.Empty>

					<Command.Group heading="Équipe du campus">
						{#each staff as person}
							<Command.Item value={person.name} onSelect={() => toggleStaff(person.id)}>
								<div class="mr-2 flex items-center justify-center">
									<Check
										class="h-4 w-4 {value.includes(person.id) ? 'opacity-100' : 'opacity-0'}"
									/>
								</div>
								<Avatar.Root class="mr-2 h-6 w-6">
									<Avatar.Image src={getAvatarUrl(person)} />
									<Avatar.Fallback class="bg-muted text-[10px] font-bold text-foreground">
