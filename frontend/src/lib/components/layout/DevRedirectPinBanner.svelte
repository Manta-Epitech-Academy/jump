...
>
  ...
  {#if untilLabel}
    <span class="font-normal text-amber-900">Se désactive à {untilLabel}.</span>
  {/if}
  <form method="POST" action="/api/dev/redirect-pin" class="inline">
    <input type="hidden" name="action" value="disarm" />
    <button
      type="submit"
      class="rounded-sm bg-amber-950/15 px-2 py-0.5 font-semibold underline-offset-2 hover:bg-amber-950/25 hover:underline"
    >
      Désactiver
    </button>
  </form>
