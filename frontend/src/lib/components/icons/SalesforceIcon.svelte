<script lang="ts">
  // Salesforce brand glyph. Drop-in for the Lucide `<Cloud />` we previously
  // used as the Salesforce marker. Filled with the official Salesforce blue so
  // it reads as the brand everywhere, regardless of surface; `class` owns the
  // box (size, margins). Text-colour utilities no longer tint it by design.
  const SALESFORCE_BLUE = '#27a1e1';
