...
<Dialog.Root bind:open>
	<Dialog.Content class="sm:max-w-106.25">
		<Dialog.Header>
			...
			<Dialog.Description>
				Configurez la copie de l'événement. Les participants seront copiés mais leur statut
				(présence, note) sera réinitialisé.
			</Dialog.Description>
		</Dialog.Header>
		...
		{#if eventToDuplicate}
			...
			>
				...
				<Dialog.Footer>
					...
					<Button type="submit" disabled={isLoading} class="bg-epi-blue text-white">
						{#if isLoading}
