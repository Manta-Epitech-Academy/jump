...
- **`attachments.ts`** - the one place a file download is composed: the two headers, and the fold of an event title down to ASCII. The folding is not a formatting preference, which is why it lives here rather than at each export route: `Content-Disposition`'s plain `filename` parameter is a latin-1 field, so an accented title arrives mangled or, depending on the client, costs the header its meaning
