...
// We store the template directly as a string to avoid fs/path issues in production builds
...
<head>
    ...
    <style>
        ...
        .header-logo { display: flex; align-items: center; justify-content: center; gap: 12px; color: #013afb; margin-bottom: 8px; }
        .header-logo svg { width: 40px; height: 40px; }
        .header-logo span { font-size: 20px; font-weight: 900; letter-spacing: 0.3em; text-transform: uppercase; }
...
export async function generateDiplomaPDF(data: {
	...
	try {
		// 1. Render HTML
		const htmlContent = await ejs.render(diplomaTemplate, { data }, { async: true });
		...
		// 3. Set content and await fonts loading
		await page.setContent(htmlContent, { waitUntil: 'networkidle0' });
		await page.evaluateHandle('document.fonts.ready');

		// 4. Generate PDF explicitly targeting the dimensions
