 ...
 * form was not collected to be quoted, unlike the closing testimonial, so it
...
export type FormResults = {
  ...
  /** Enrolments that could have answered: the denominator of `responseRate`. */
  invited: number;
...
async function resultsFor(
  ...
  return {
    ...
    connectedShare: share(stats.authSubmissions, stats.totalSubmissions),
    publicSubmissions: stats.publicSubmissions,
    publicShare: share(stats.publicSubmissions, stats.totalSubmissions),
    invited,
