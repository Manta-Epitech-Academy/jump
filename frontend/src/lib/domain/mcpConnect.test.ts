...
// Shaped like a real secret (the `jump_` prefix, and base64url's `-` and `_`,
// which is what lets the command quote it without escaping) but deliberately
// not entropic: gitleaks reads a random-looking literal next to the word token
// as a leak, and a fixture that trips the secret scanner on every push teaches
// people to wave the scanner through.
const TOKEN = 'jump_EXAMPLE-NOT-A-SECRET_EXAMPLE';
