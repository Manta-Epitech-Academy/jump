...
<div class="space-y-6">
  ...
  <Tabs.Root
    value={sourceFilter}
    onValueChange={(v) => (sourceFilter = v)}
    class="w-full"
  >
    <div
    ...
    >
      ...
      <div
        class="hidden items-center gap-1 rounded-md border bg-background p-1 md:flex"
      >
    ...
    <Dialog.Content class="max-h-[90vh] overflow-y-auto sm:max-w-xl">
      ...
      >
        ...
        <div class="grid gap-3 rounded-md border bg-muted/20 p-4">
          ...
          {#if $errors.difficulte}<span class="text-xs text-destructive"
