...
export const actions: Actions = {
  ...
  analyzeCampaign: async ({ request, locals }) => {
    ...
    try {
      ...
      const { eventName, eventDate, students } =
        await parseEventImportCsv(text);
          ...
          try {
            ...
            if (status === 'NEW') {
              try {
                ...
                existing = nameMatch;
                reason =
                  'Nom identique mais email différent (Homonyme possible)';
              } catch (___) {
                /* Truly new person */
              }
            }
