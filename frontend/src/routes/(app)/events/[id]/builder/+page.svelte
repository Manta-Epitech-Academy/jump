                  ...
                  <div class="flex gap-2">
                    <Select.Root type="single" bind:value={hour}>
                      ...
                      <Select.Content class="h-50">
                        {#each hours as h}<Select.Item value={h}
                            >{h}</Select.Item
                          >{/each}
                      </Select.Content>
                    </Select.Root>
                    <Select.Root type="single" bind:value={minute}>
                      ...
                      <Select.Content>
                        {#each minutes as m}<Select.Item value={m}
                            >{m}</Select.Item
                          >{/each}
                      </Select.Content>
                    </Select.Root>
            ...
            >
              <div class="space-y-1">
                ...
                <p class="text-xs text-muted-foreground">
                  La suppression d'un événement est irréversible. Les XP des
                  élèves validés seront automatiquement retirés.
                </p>
              </div>
              <Button
                type="button"
        ...
        <Card.Header class="space-y-4 pb-3">
          <div class="flex items-center justify-between">
            ...
            <Dialog.Root bind:open={openQuickCreate}>
              ...
              <Dialog.Content>
                <Dialog.Header
                  ><Dialog.Title>Ajout Rapide</Dialog.Title></Dialog.Header
                >
                <form
                  method="POST"
                ...
                >
                  ...
                  <div class="space-y-2">
                    ...
                    >
                      <Select.Trigger
                        >{$createForm.niveau || 'Sélectionner'}</Select.Trigger
                      >
                      <Select.Content
                        >{#each niveauxScolaires as nv}<Select.Item value={nv}
                            >{nv}</Select.Item
                          >{/each}</Select.Content
                      >
                    </Select.Root>
