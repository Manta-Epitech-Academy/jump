<script lang="ts">
  ...
  let isDynamic = $derived(data.activity.isDynamic);
  ...
  let currentIndex = $derived(
    isDynamic ? steps.findIndex((s) => s.id === progress?.currentStepId) : -1,
  );
  ...
  let currentStep = $derived(
    isDynamic ? steps[currentIndex] || steps[0] : null,
  );
    ...
    <main class="flex-1 overflow-y-auto bg-white dark:bg-slate-900">
      ...
      >
        {#if data.activity.description}
          <p class="mb-6 text-sm text-slate-500">{data.activity.description}</p>
        {/if}
...
{:else}
  ...
  >
    ...
    <div class="relative flex flex-1 overflow-hidden">
          ...
          >
            ...
            {:else}
              <p class="mb-6 max-w-md text-sm text-slate-500">
                Excellent travail. Avant de partir, dis-nous ce que tu as pensé
                de cette mission :
              </p>
              <form
                method="POST"
      ...
      <MantaSignalButton progress={progress!} {isCompleted} />
    </div>
