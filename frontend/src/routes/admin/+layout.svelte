<script lang="ts">
	...
	function isActive(path: string) {
		...
		const fullPath = path === '/admin' ? `${basePath}/admin` : `${basePath}${path}`;
		if (path === '/admin') {
...
<div class="flex h-screen w-full flex-col overflow-hidden bg-background">
				...
				<DropdownMenu.Root>
					...
					>
						<div class="hidden flex-col items-end md:flex">
							...
							<span class="text-sm font-bold text-slate-200">{data.user?.email}</span>
						</div>
						<div
	...
	<div class="relative flex flex-1 overflow-hidden">
		...
		<aside class="hidden w-64 flex-col border-r border-slate-800 bg-slate-950 md:flex">
			<div class="flex-1 overflow-y-auto py-6 pr-4">
				<div class="mb-2 px-6 text-[10px] font-black tracking-widest text-slate-500 uppercase">
				...
				<nav class="mb-8 space-y-1">
					<a href={resolve('/admin/campuses')} class={navLinkClass(isActive('/admin/campuses'))}>
						<Map class="h-4 w-4" />
						<span>Réseau Campus</span>
					</a>
					<a href={resolve('/admin/users')} class={navLinkClass(isActive('/admin/users'))}>
						<Users class="h-4 w-4" />
				...
				<nav class="space-y-1">
					<a href={resolve('/admin/subjects')} class={navLinkClass(isActive('/admin/subjects'))}>
						<BookOpen class="h-4 w-4" />
						<span>Sujets Officiels</span>
					</a>
					<a href={resolve('/admin/themes')} class={navLinkClass(isActive('/admin/themes'))}>
						<Tags class="h-4 w-4" />
