...
<div class="space-y-6">
			...
			<Table.Body>
				{#each data.subjects as subject}
					<Table.Row>
						...
						<Table.Cell>
							<Badge variant="outline" class="text-[10px] uppercase">{subject.difficulte}</Badge>
	...
	<Dialog.Root bind:open>
		<Dialog.Content class="sm:max-w-xl">
			<Dialog.Header>
				<Dialog.Title>{isEditing ? 'Modifier le sujet' : 'Créer un sujet OFFICIEL'}</Dialog.Title>
			</Dialog.Header>
			<form
				method="POST"
			...
			>
				...
				<div class="grid gap-2">
					...
					<Textarea name="description" bind:value={$form.description} class="h-24" />
				</div>
				<Dialog.Footer>
