...
export const actions: Actions = {
	requestOtp: async ({ request, locals }) => {
		...
		try {
			...
			return message(emailForm, { type: 'success', text: 'Code envoyé', otpId: result.otpId });
		} catch (err) {
			...
			if (err instanceof ClientResponseError) {
				if (err.status === 429) {
					return message(
						emailForm,
						{ type: 'error', text: 'Trop de tentatives. Veuillez patienter avant de réessayer.' },
						{ status: 429 }
					);
				}
			}
			...
			return message(
				emailForm,
				{ type: 'error', text: "Impossible d'envoyer le code. Réessayez plus tard." },
				{ status: 500 }
			);
		}
	},
