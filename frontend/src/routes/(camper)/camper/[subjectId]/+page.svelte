          ...
          >
            ...
            >
              ...
              {:else if currentStep.validation?.type === 'auto_qcm' && currentStep.validation.qcm_data}
                <div class="space-y-6">
                  <div class="flex items-center gap-2">
                    ...
                    <h3
                      class="font-bold text-slate-900 uppercase dark:text-white"
                    >
                      Validation Requise
                    </h3>
                  </div>
                        ...
                        if (result.type === 'success') {
                          ...
                          toast.error(
                            ((result.data as Record<string, unknown>)
                              ?.message as string) || 'Mauvaise réponse.',
                          );
                        }
                        await update({ reset: false });
                      };
              ...
              {:else if currentStep.validation?.type === 'manual_manta'}
                ...
                >
                  <div
                    class="mb-4 rounded-full bg-orange-50 p-4 dark:bg-orange-950/30"
                  >
                    <Lock class="h-8 w-8 text-epi-orange" />
                  </div>
                  <h3
                    class="mb-2 font-bold text-slate-900 uppercase dark:text-white"
                  >
                    Validation Manta
                  </h3>
                  <p class="mb-6 max-w-sm text-sm text-slate-500">
                    Montre ton travail, puis clique sur "Appeler un Manta". Le
                    Manta validera à distance.
                  </p>
                  ...
                  >
                    <p
                      class="mb-3 text-[10px] font-bold text-slate-400 uppercase"
                    >
                      ...
                      use:enhance={() => {
                        ...
                        return async ({ result, update }) => {
                          ...
                          if (result.type === 'success') {
                            ...
                            toast.error(
                              (result as any).data?.message || 'PIN Incorrect',
                            );
                          }
                          await update({ reset: false });
                        };
    ...
    <div class="absolute right-6 bottom-6 z-30">
      ...
      >
        ...
        >
          ...
          {:else}
            <LifeBuoy
              class={cn('h-5 w-5', isCompleted ? '' : 'animate-pulse')}
            />
