...
<div class="mx-auto max-w-4xl space-y-6 p-6">
  <div>
    <h1 class="text-2xl font-bold">Page d'accueil des participants</h1>
    <p class="mt-1 text-sm text-muted-foreground">
      Ce contenu est la première chose que les participants voient en arrivant
      sur la plateforme. Il s'affiche une seule fois, avant l'accès au tableau
      de bord. Utilisez-le pour présenter le programme, donner des consignes ou
        ...
        return async ({ result, update }) => {
          ...
          if (result.type === 'success') {
            track('cms_welcome_saved', {
              space: 'dev',
              bodyLength: bucketLength(content.length),
            });
          }
          await update();
        };
