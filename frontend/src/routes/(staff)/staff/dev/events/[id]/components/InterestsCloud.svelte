...
<Card.Root class="rounded-sm shadow-sm dark:shadow-none">
  <Card.Header class="border-b bg-muted/30 pt-4 pb-3">
    ...
    >
      <Sparkle class="h-4 w-4 text-epi-pink" />
      ...
      <div class="flex flex-wrap gap-2">
        {#each sized as i (i.interestId)}
          <span
            ...
            style="font-size: {i.fontSize}px; background: rgba(155,11,147,{i.bgAlpha});"
          >
            {i.label}
            <span class="font-mono text-[10px] font-bold text-epi-pink">
              {i.count}
            </span>
          </span>
        {/each}
