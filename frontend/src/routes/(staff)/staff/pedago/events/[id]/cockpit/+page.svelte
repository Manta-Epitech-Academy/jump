...
<div class="flex min-h-[calc(100vh-4rem)] flex-col bg-background">
  <div class="border-b border-border bg-slate-950 pt-6 pb-6 text-white">
    <div class="container mx-auto max-w-3xl px-4">
      ...
      <h1 class="mt-4 flex items-center gap-3 text-2xl font-bold uppercase">
        <MonitorPlay class="h-6 w-6 text-epi-blue" />
        Sélecteur de Cockpit
      </h1>
      <p class="mt-2 text-sm font-medium text-slate-300">
        L'événement <strong class="text-epi-blue">{data.event.titre}</strong> comporte
        plusieurs sessions. Quel créneau souhaitez-vous piloter ?
      </p>
    </div>
  </div>
    ...
    <div class="space-y-4">
      {#each data.orgaActivities as orga (orga.id)}
        <a
          ...
          class="flex items-center gap-4 rounded-xl border bg-card p-5 transition-all hover:-translate-y-1 hover:border-epi-blue hover:shadow-md"
        >
          <div
            class="flex h-12 w-12 shrink-0 items-center justify-center rounded-lg bg-epi-blue/10"
          >
            <MonitorPlay class="h-6 w-6 text-epi-blue" />
          </div>
          <div class="flex-1">
            <span class="text-lg font-bold">{orga.nom}</span>
            <div
            ...
            >
              ...
              {#if orga.slotLabel}
                <span class="ml-2 rounded-sm bg-muted px-2 py-0.5 text-xs"
                  >{orga.slotLabel}</span
                >
              {/if}
            </div>
