<script lang="ts">
  ...
  // ── Search ──────────────────────────────────────────────────────────────
  // One box filters every tab's list by name or email.
  let query = $state('');
  ...
  const visibleAuth = $derived(
    data.authConflicts.filter((c) => matches(c.prenom, c.nom, c.targetEmail)),
  );
  const authCount = $derived(data.authConflicts.length);
...
<div class="space-y-6">
  ...
  <Tabs.Root value={authExposureCount > 0 ? 'auth' : 'data'} class="w-full">
    ...
    <Tabs.Content value="auth" class="space-y-4 pt-4">
      <p class="max-w-3xl text-sm text-muted-foreground">
        Le compte de connexion d'un talent ne porte plus l'email avec lequel il
        se connecte (changement / inversion Salesforce). Chaque ligne propose la
