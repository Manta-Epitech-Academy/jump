<script lang="ts">
  ...
  const chartQuestions = $derived(
    (data.stats?.questions ?? []).filter(
      (q) => q.type !== 'text' && q.type !== 'textarea',
    ),
  );
  const textQuestions = $derived(
    (data.stats?.questions ?? []).filter(
      (q) => q.type === 'text' || q.type === 'textarea',
    ),
  );
...
<div class="space-y-6 pb-16">
  ...
  <div class="flex flex-wrap items-center justify-between gap-4">
    ...
    <div class="flex items-center gap-4">
      <p class="text-sm text-muted-foreground">
        ...
        réponses
        <span class="text-xs"
          >({data.stats?.authSubmissions ?? 0} authentifiées ·
          {data.stats?.publicSubmissions ?? 0} publiques)</span
        >
      </p>
      <a
        href={exportHref}
    ...
    <div class="space-y-4">
      ...
      {#each textQuestions as q (q.questionId)}
        <section class="rounded-sm border bg-card p-4">
          ...
          {:else}
            <p class="text-sm text-muted-foreground italic">Aucune réponse.</p>
          {/if}
        </section>
      {/each}
    </div>
  ...
  {#if data.publicRespondents.length > 0}
    <section class="space-y-3 rounded-sm border bg-card p-4">
      ...
      <Table.Root>
        ...
        <Table.Body>
          {#each data.publicRespondents as r (r.id)}
            <Table.Row>
              <Table.Cell class="font-mono text-xs">{r.email ?? '—'}</Table.Cell
              >
