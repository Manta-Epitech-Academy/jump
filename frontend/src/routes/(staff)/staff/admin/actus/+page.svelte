...
<div class="mb-6">
  <AdminPageHeader title="Gestion des" accent="actualités" cursor />
  <p class="mt-1 max-w-3xl text-sm text-muted-foreground">
    Visualisez et gérez les actualités de tous les campus. Vous pouvez créer des
    actualités globales (tous les campus) ou ciblées sur un campus spécifique.
  </p>
</div>
          ...
          {#if newCampusEvents.length > 0}
            <div>
              <span class="mb-1 block text-sm font-medium"
                >Événement associé (optionnel)</span
              >
              <input
                type="hidden"
                name="eventId"
  ...
  {#if filteredPosts.length === 0 && !creating}
    <div class="rounded-lg border border-dashed border-border p-8 text-center">
      <p class="text-muted-foreground">
        Aucune actualité pour cette sélection.
      </p>
    </div>
  {/if}
