...
>
	...
	<div class="z-10 w-full max-w-md">
		...
		>
			<div class="absolute -top-3 -right-3 h-6 w-6 rotate-45 bg-epi-pink opacity-80"></div>
			...
			<Card.Header class="space-y-4 pb-2 text-center">
				...
				<div class="space-y-1">
					<Card.Title class="font-heading text-2xl tracking-wide text-white uppercase">
			...
			<Card.Content class="space-y-6 pt-6">
				...
				{#if $message}
					<Alert variant="destructive" class="border-red-500/50 bg-red-950/50 text-red-200">
						<ShieldAlert class="h-4 w-4" color="#f87171" />
						<AlertTitle class="text-[11px] font-bold tracking-widest text-red-300 uppercase">
							Alerte de sécurité
						</AlertTitle>
				...
				<form method="POST" use:enhance class="space-y-4">
					<div class="space-y-2">
						...
						/>
						{#if $errors.email}<span class="text-xs text-red-400">{$errors.email}</span>{/if}
					</div>
					...
					<div class="space-y-2">
						...
						<Input
							...
							class="border-slate-700 bg-slate-800 text-white placeholder:text-slate-500 focus-visible:ring-epi-pink"
						/>
						{#if $errors.password}<span class="text-xs text-red-400">{$errors.password}</span>{/if}
					</div>
				...
				>
					...
					<span>
						Cet espace est réservé à l'administration technique (Gestion des campus, Base de
						connaissances globale). L'accès y est formellement surveillé.
					</span>
				</div>
			</Card.Content>
		</Card.Root>
...
<style>
	...
	.admin-login-bg::after {
		...
		width: 100vw;
		height: 100vh;
		background: linear-gradient(to bottom, transparent, rgba(255, 30, 247, 0.03), transparent);
