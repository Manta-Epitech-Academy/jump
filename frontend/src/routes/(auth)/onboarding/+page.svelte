...
>
  ...
  <Card.Root
    class="z-10 w-full max-w-md border-t-4 border-t-epi-blue shadow-2xl"
  >
      ...
      <form method="POST" action="?/joinCampus" use:enhance class="space-y-6">
        <div class="space-y-2">
          <Select.Root type="single" bind:value={selectedCampusId}>
            ...
            <Select.Trigger
              class="flex h-12 w-full items-center justify-center text-base"
            >
              {selectedCampusName}
            </Select.Trigger>
            ...
            <Select.Content
              class="max-h-60 w-[var(--bits-select-anchor-width)] overflow-y-auto"
            >
